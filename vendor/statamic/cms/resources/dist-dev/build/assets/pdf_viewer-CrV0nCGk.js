...
const _AppOptions = class _AppOptions {
  static get(name) {
...
class Autolinker {
  ...
  static findLinks(text) {
    this.#regex ??= new RegExp("\\b(?:https?:\\/\\/|mailto:|www\\.)(?:[\\S--[\\p{P}<>]]|\\/|[\\S--[\\[\\]]]+[\\S--[\\p{P}<>]])+|(?=\\p{L})[\\S--[@\\p{Ps}\\p{Pe}<>]]+@([\\S--[[\\p{P}--\\-]<>]]+(?:\\.[\\S--[[\\p{P}--\\-]<>]]+)+)", "gmv");
    ...
    for (const match2 of matches) {
      ...
      if (absoluteURL) {
        const [index, length] = getOriginalIndex(diffs, match2.index, url.length);
        links.push({
...
class BasePDFPageView {
  ...
  async _drawCanvas(options, onCancel, onFinish) {
    ...
    renderTask.onError = (error2) => {
      if (error2 instanceof RenderingCancelledException) {
        onCancel();
...
const MathMLElements = /* @__PURE__ */ new Set(["math", "merror", "mfrac", "mi", "mmultiscripts", "mn", "mo", "mover", "mpadded", "mprescripts", "mroot", "mrow", "ms", "mspace", "msqrt", "mstyle", "msub", "msubsup", "msup", "mtable", "mtd", "mtext", "mtr", "munder", "munderover", "semantics"]);
